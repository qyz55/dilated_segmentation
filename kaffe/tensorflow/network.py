...
class Network(object):
    ...
    def atrous_conv(self,
        ...
        assert c_i % group == 0
        assert c_o % group == 0
        # Convolution for a given input and 
    ...
    @layer
    def div(self, inputs, name):
        return tf.divide(inputs[0], inputs[1] , name=name)

    @layer
    def exp(self, inputs, name):
        return tf.exp(inputs, name = name)

    @layer
    def mult(self, inputs, name):
